...
% Last Modified by GUIDE v2.5 04-May-2012 18:30:47
...
pathToAdd = which('wave_clus');
pathToAdd = pathToAdd(1:end-(length('wave_clus.m')+1));
...
for i=1:length(xx)
    temp=xx(i,1).name; 
    if (~isequal(temp, '.') && ~isequal(temp, '..')) && isdir([pathToAdd '\' temp])
...
% commonFilePath = ffpath('closeGUI');

% fullPath = [fullPath commonFilePath ';'];

addpath(fullPath);

fh = findobj('Tag', 'wave_clus_figure');
setappdata(fh, 'addedPath',fullPath);

set(handles.wave_clus_figure, 'CloseRequestFcn', 'closeGUI');
...
switch char(handles.datatype)
    case 'Simulator'  
        ...
        x.data=data;
        x.sr=20000;%1./samplingInterval; %in case of transistor recordings - x.sr=20000;
...
%          s=d(:,2);
%          dt=(t(2)-t(1));
%          time=linspace(0,length(t)*dt-dt,length(t))';
%          Fs=1/dt;
%          cutoffFreq=1000;
...
if icomp == 0  
    eval(['copyfile([handles.par.fnamespc ''.dg_01.lab''],[handles.par.fnamesave ''.dg_01.lab''])'],'[];');
    eval(['copyfile([handles.par.fnamespc ''.dg_01''],[handles.par.fnamesave ''.dg_01''])'],'[];');
end
