function [sigma, sourceDiameter, recordingDepths, csdValues, t]=splineInverseCSD(directoryPath, fileNames, recordingDepths, sigma, sourceDiameter, checkedColumns, stimOnset)
...
%     figureHandle=plotDepthProfile(filteredPotentialsForCSD,t,'Filtered LFPs', recordingDepths);%'amplitude[mv]');
    figureHandle=plotDepthProfile(pltPot, t(ceil(Fs*0.001*stimOnset)+1:length(t)),'Filtered LFPs', recordingDepths);

    figureHandle=plotDepthProfile(-1.*pltCSD,t(ceil(Fs*0.001*stimOnset)+1:length(t)),'Calculated CSD using Spline iCSD',recordingDepths);%'amplitude[uA/mm^3]');
