function[t_SS,v_SS,t_RS,v_RS,t_P1,v_P1,l_P1,t_P2,v_P2,l_P2,t_P3,v_P3,l_P3,t_P4,v_P4,l_P4,s,t,flag] =  detectLatencyInAvgV5StimOnset(filePath, fileName, cutoffFreq, signalType)
    ...
    if (signalType==2) % if the traces are of micropipette recording
        z = data(:,2);
    elseif (signalType==3) && (abs(min(data(:,2)))<0.01) % if the traces are of transistor recording
...
%     plot(respOnsetT,respOnsetV,'ro');    
    ...
    if (s(posRespOnset+2*dataPoints) > stdSteady*2) || (max(s(posRespOnset:round(posRespOnset+Fs*0.015)))>0)
        ...
        peakMax = max(s(posRespOnset:round(posRespOnset+Fs*0.01)));
        if peakMax > 0.015
...
%     posMin = posS + min(posMin);
    ...
    if(t(posMin)<t(posS)+0.1)
        ...
        % for doing so, consider the first 150 ms after the negative peak

        z_scaled_cut = s(posMin:round(posMin+Fs*0.15));
