function layerActivationOrderByLFP(latMat, folderName)
...
%     minLat(7)=mean(layerVILat);
    ...
    figure('Name','SigMate: Layer Activation Order using LFPs','NumberTitle','off');
    plot(layAct(:,2),'rO-')
    set(gca,'XTickLabel',actLayers)
    xlabel('Cortical Layers')
    ylabel('signal propagation latency [ms]')
    title(['Layer Activation Order in Barrel Cortex: ' folderName])