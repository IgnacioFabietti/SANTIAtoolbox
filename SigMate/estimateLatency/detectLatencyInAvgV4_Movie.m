% function[t_SS,v_SS,t_RS,v_RS,t_P1,v_P1,l_P1,t_P2,v_P2,l_P2,t_P3,v_P3,l_P3,t_P4,v_P4,l_P4,s,t,flag] =  detectLatencyInAvgV4_Movie(filePath, fileName, cutoffFreq, signalType)

%     filePath='E:\In-Vivo Recordings\Pipette Recordings Whisker Stim\20090715_P37_150gr_male-analyzed\20090715-NewPosition-P37-150gr-Male\TXT\';%'E:\In-Vivo Recordings\Pipette Recordings Whisker Stim\20090715_P37_150gr_male-analyzed\20090715-NewPosition-P37-150gr-Male\TXT\';
    signalType=2;
    cutoffFreq=200;
...
%     fileNames=getFileNames(filePath);
    ...
    aviObj=avifile('example-10fps.avi','FPS',10);
    ...
    %             hold off
            ...
            chunkSize=300;
            chunkNum=floor(length(t)/chunkSize);
            fn=1;
    %         S1=sprintf('plot for:%s',fileName);
    %         text(2,.6,S1)
            title(['LFP and its latencies for depth: ' depth(index,:)])
            xlabel('time [ms]')
            ylabel('amplitude [mV]')
            cla
%             for i=0:chunkNum-1
%                 plot(t((chunkSize*i)+1:(chunkSize*(i+1))),s((chunkSize*i)+1:(chunkSize*(i+1))))
%                 M=getframe(fig);
%                 aviObj = addframe(aviObj,M);            
%     %             (chunkSize*(i+1))
%             end

            plot(t,s);%((chunkSize*i)+1:(chunkSize*(i+1))),s((chunkSize*i)+1:(chunkSize*(i+1))))
            M=getframe(fig);
            aviObj = addframe(aviObj,M);    
...
%         signalMatrix(:,index)=s;
%         maxAmplitude(index)=abs(min(s));
...
%             normFactor=0;
%         else
%             tempData=load([filePath fileNames{index-1}]);
%             prevS=tempData(:,2);
%             % low pass filter the signal with the specified cutoff frequency
%             fNorm = cutoffFreq / (Fs/2);
%             [b,a] = butter(5,fNorm,'low');
%             prevS = filtfilt(b,a,prevS);            
%             [newS, normFactor]=normalizeSignal(prevS, s, normFactor);
%         end
%         
%         
%         chunkSize=500;
...
%         text(2,.6,S1)
%         for i=0:chunkNum-1
%             plot(t((chunkSize*i)+1:(chunkSize*(i+1))),newS((chunkSize*i)+1:(chunkSize*(i+1))))
%             M(fn)=getframe;
%             fn=fn+1;
% %             (chunkSize*(i+1))
%         end
...
%     normFactor=max(maxAmplitude)/3;
    
%     for index=1:length(fileNames)
%         data=load([filePath fileNames{index}]);
%         s=data(:,2);
%         % low pass filter the signal with the specified cutoff frequency
%         fNorm = cutoffFreq / (Fs/2);
%         [b,a] = butter(5,fNorm,'low');
