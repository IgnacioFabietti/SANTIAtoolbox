function[mat_coordinate_clu_new, mat_coordinate_clu_old, mat_assegnamenti_finali, num_clu_sel] = funz_clustering_som(mat_data)
...
max_iter = input('Insert the number of iterations, (default = 10000): ');

eta_max = input('Insert the value of the parameter eta, (default = 1.5): ');
...
for i = 1:num_iterazioni
    ...
    sgn_selected_tmp = funz_sgn_selection(vet_indici);
    sgn_tmp = mat_coordinate_sgn(sgn_selected_tmp,:);
    
    % 2. selection of the closer neuron to the selected signal
    winner = funz_closer_clu(sgn_tmp, mat_coordinate_clu, measure);
    
    % 3. modification of the position of the winner and of its neighbors
    mat_coordinate_clu = funz_aggiornamento_new(sgn_tmp, winner, mat_coordinate_clu, eta);
        
end

mat_assegnamenti_finali = funz_assegnamento_sgn(mat_coordinate_sgn, mat_coordinate_clu, measure);
