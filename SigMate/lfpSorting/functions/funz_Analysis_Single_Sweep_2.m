function[res1, res2, res3, res4] = funz_Analysis_Single_Sweep_2(Signal_Matrix)
...
Segnale_BIG_filtrato_posS_0 = funz_gen_sgn_big_filt_posS_0(TF_Matrix);
...
for c = 1:nr
    ...
    pos_best_next_to_zero = funz_ricerca_fine_risposta(sgn_tmp, vett_MinMax_ord,m);
...
switch method_to_use
    ...
    case 2
        ...
        x = 1;
        for i=9984:9984:length(Segnale_BIG)
            pos = i;
            if (length(find(istant_time_ok == pos))==1)&(length(find(pos_sf == pos))==1)
                VP_num = VP_num+1;
                single_sweep_recognized(x) = i;
                x = x+1;
            elseif (length(find(istant_time_ok == pos))==1)
                FP_num = FP_num+1;
            elseif (length(find(pos_sf == pos))==1)
                FN_num = FN_num+1;
            else
                VN_num = VN_num+1;
            end    
...
switch clustering_type
    ...
    % K-means
    case 1
        [mat_coordinate_clu_best, mat_coordinate_clu_old_best, mat_appartenenze_best, K] = fun_clustering_kmeans(Parameters_matrix);
...
%                 title(['Sweeps part of the cluster ', num2str(clu_num)])
                ...
                plot(t, summation, 'r', 'LineWidth',2)
    ...
    % SOM    
    case 2
        [mat_coordinate_clu_new, mat_coordinate_clu_old, mat_assegnamenti_finali, K] = funz_clustering_som(Parameters_matrix);
    ...
    case 3
        [cluster_mat, cluster_vet_stop, vect_distance, K, mat_dendrogram] = funz_clustering_agglomerative(Parameters_matrix);
