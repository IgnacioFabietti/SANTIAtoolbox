...
% ---------
if ~iseeglabdeployed2
    addedpath=myaddpath( eeglabpath, 'eeg_checkset.m',   [ 'functions' filesep 'adminfunc'        ]);
    addedPaths{1}=[addedpath ';'];
    eeglab_options;
    addedpath=myaddpath( eeglabpath, 'memmapdata.m', 'functions');
    addedPaths{2}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'readeetraklocs.m', [ 'functions' filesep 'sigprocfunc'      ]);
    addedPaths{3}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'supergui.m',       [ 'functions' filesep 'guifunc'          ]);
    addedPaths{4}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'pop_study.m',      [ 'functions' filesep 'studyfunc'        ]);
    addedPaths{5}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'pop_loadbci.m',    [ 'functions' filesep 'popfunc'          ]);
    addedPaths{6}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'statcond.m',       [ 'functions' filesep 'statistics'       ]);
    addedPaths{7}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'timefreq.m',       [ 'functions' filesep 'timefreqfunc'     ]);
    addedPaths{8}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'icademo.m',        [ 'functions' filesep 'miscfunc'         ]);
    addedPaths{9}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'eeglab1020.ced',   [ 'functions' filesep 'resources'        ]);
    addedPaths{10}=[addedpath ';'];
    addedpath=myaddpath( eeglabpath, 'startpane.m',      [ 'functions' filesep 'javachatfunc' ]);
    addedPaths{11}=[addedpath ';'];
    ...
    addedpath=myaddpath( eeglabpath, 'eegplugin_dipfit', 'plugins');
    addedPaths{12}=addedpath;
else
    eeglab_options;
end;
...
if iseeglabdeployed2
    ...
    dircontent  = { dircontent.name }; xx=1;
    for index = 1:length(dircontent)
        if dircontent{index}(1) ~= '.'
            if exist([p 'external' filesep dircontent{index}]) == 7
                addpath([p 'external' filesep dircontent{index}]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} ';']; xx=xx+1;
                disp(['Adding path to eeglab' filesep 'external' filesep dircontent{index}]);
            end;
            if ~isempty(findstr('fieldtrip', lower(dircontent{index})))
                addpathexist([p 'external' filesep dircontent{index} filesep 'forward' ]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} filesep 'forward' ';']; xx=xx+1;
                addpathexist([p 'external' filesep dircontent{index} filesep 'inverse' ]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} filesep 'inverse' ';']; xx=xx+1;
                addpathexist([p 'external' filesep dircontent{index} filesep 'utilities' ]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} filesep 'utilities' ';']; xx=xx+1;
                addpathexist([p 'external' filesep dircontent{index} ]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} ';']; xx=xx+1;
            ...
            if ~isempty(findstr('biosig', lower(dircontent{index})))
                addpathexist([p 'external' filesep dircontent{index} filesep 't200_FileAccess' ]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} filesep 't200_FileAccess;']; xx=xx+1;
                addpathexist([p 'external' filesep dircontent{index} filesep 't250_ArtifactPreProcessingQualityControl' ]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} filesep 't250_ArtifactPreProcessingQualityControl;']; xx=xx+1;
                addpathexist([p 'external' filesep dircontent{index} filesep 'doc' ]); addedPaths{12+xx}=[p 'external' filesep dircontent{index} filesep 'doc;']; xx=xx+1;
...
cb_quit        = [  'W_MAIN = findobj(''tag'', ''EEGLAB''); addedPaths=getappdata(W_MAIN,''addedPaths''); closeGUI(); paths=path; for i = 1: length(addedPaths), if strfind(paths,addedPaths{i}), rmpath(addedPaths{i}); end, end, disp(''To save the EEGLAB command history  >> pop_saveh(ALLCOM);'');' ...
                   'clear global EEG ALLEEG LASTCOM CURRENTSET;'];
    ...
    for index = 1:length(dircontent)
        ...
        if exist([p 'plugins' filesep dircontent{index}]) == 7
            if ~strcmpi(dircontent{index}, '.') & ~strcmpi(dircontent{index}, '..')
                addpath([ p 'plugins' filesep dircontent{index} ]); addedPaths{12+xx}=[p 'plugins' filesep dircontent{index} ';']; xx=xx+1;
...
eval('set(W_MAIN, ''CloseRequestFcn'', ''close_msg_eeglab'');');
eeglab('redraw');
setappdata(W_MAIN,'addedPaths',addedPaths);
if nargout < 1
    clear ALLEEG;
end;
...
%     msgbox('Please use the File->Quit to close!','Close Disabled','Error')
% end
...
% ------------------------------------------------
function addedpath=myaddpath(eeglabpath, functionname, pathtoadd)
